...
shape. In contrast to sparse shape recovery, dense shape recovery generally
attempts to estimate the geometry for every pixel that is present within
the input image. This is a far more challenging problem than the sparse case
as it requires a more detailed understanding of the scene. One common method
for achieving detailed scene understanding is to match areas of an image with
areas of a known model of a scene. This is highly effective for sparse shape
recovery as it is generally possible to find salient regions that can be
matched precisely. However, this is not very effective for dense shape recovery
as the cost of this matching may be prohibitive. Furthermore, most objects have
smooth textureless regions where any attempt at correspondence becomes highly
ambiguous. For these reasons, dense shape recovery is widely regarded as a more
challenging problem than sparse shape recovery. In this thesis we
consider only the problem of dense 3D shape recovery.
...
\begin{table*}[t]
	...
	\resizebox{\textwidth}{!}{%
		...
		\multirow{4}{*}[-0.7cm]{Image Formation}            & Shape-from-Shading                                                           & $1$       & \begin{tabular}[c]{@{}c@{}}Reflectance Model, \\ Lighting Directions\end{tabular} & Surface Normals or Depth Map \\ \cmidrule(l){3-5}
		                                                    & \begin{tabular}[c]{@{}l@{}} (Un) Calibrated \\ Photometric Stereo\end{tabular} & $\geq 3$  & \begin{tabular}[c]{@{}c@{}}Lighting Directions \\ (If Calibrated)\end{tabular}  & Surface Normals or Depth Map \\ \cmidrule(l){3-5}
...
under ideal conditions (near-frontal face, no expression, young Caucasian subject).
As previously mentioned and summarised
