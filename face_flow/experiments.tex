...
error of the endpoints (RMSE), shown in \cref{tbl:synthetic}. As expected
...
set of endpoints is given in \cref{fig:synthetic_illum_examples}. Note how
...
of the low-rank constraint on the outcome of the sequence, we present \cref{fig:per_frame_error}.
...
As \cref{fig:real_examples} shows, Face Flow LR performs very well in this sequence. In particular,
we note that our method is very robust to the presence of occlusions, aided by the sparse
landmarks provided by~\cite{kazemi2014one}. We also provide \cref{tbl:run_times}
...
1.4   & 0.5   & 20   & 15   & $\sim$50 & 15       \\ \bottomrule
    ...
    \caption{Run times \textit{per frame (in seconds)} for the 150 frame
             sequence shown in \cref{fig:real_examples}. Images are
