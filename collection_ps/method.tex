...
In fact, this is the exact decomposition problem
...
\begin{equation}\label{eq:text_B_update2}
    \tsub{A} {\left(\tsub{L} \ast \tsub{C} \right)}^{\top} = \bb{U} \bb{\Sigma} \bb{V^{\top}},
\end{equation}
%%%%%%%%%%%%%%%%%%%
being the SVD of $\tsub{A} {\left(\tsub{L} \ast \tsub{C} \right)}^{\top}$. However, due the
...
solved as follows:
Let $\bb{Q} = \tsub{B^{\top}} \tsub{A}, \bb{L} = \tsub{L}, \text{and} \; \bb{C} = \tsub{C}$.
...
of normals provided by a known mean face. After recovering the image-specific
spherical harmonics from the model,
$\bb{Q} = \bb{S} \times_2 \bb{c}_i \in \R^{d \times 4}$, we can take the
last three columns of the spherical harmonics which represent the normals.
We can then rotate them according to a set of reference normals by solving a
linear least squares problem which recovers a rotation matrix that aligns the
normals to stop permutations of the normals axes. We resove the GBR ambiguity
by using a reference depth map and similar to the rotation ambiguity for normals
we solve the sparse linear least squares problem that finds the Affine
correction that accounts for the GBR. This is applied after integration
