...
%%%%%%%%%%%%%%%%%%%
\newcommand{\txx}[1]{{\bb{#1}_{[t]}}}
\newcommand{\tpxx}[1]{{\bb{#1}_{[t+1]}}}
%%%%%%%%%%%%%%%%%%%
...
\begin{equation}\label{eq:text_B_update2}
    \tpxx A {\left(\txx L \ast \txx C\right)}^{\top} = \bb{U} \bb{\Sigma} \bb{V^{\top}},
\end{equation}
%%%%%%%%%%%%%%%%%%%
being the SVD of $\tpxx A {\left(\txx L \ast \txx C\right)}^{\top}$. However, due the
...
solved as follows:
Let $\bb{Q} = \tpxx B^{\top} \tpxx A, \bb{L} = \txx L, \text{and} \; \bb{C} = \txx C$.
